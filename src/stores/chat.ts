...
export const useChatStore = defineStore('chat', () => {
  ...
  const currentConversationId = ref<string>(localStorage.getItem('nextchat-current-conversation') || '1')
  ...
  const extractTitleFromMessage = (content: string): string => {
    // 移除特殊字符和表情符号
    const cleanContent = content.replace(/[^\u4e00-\u9fa5\u3040-\u309f\u30a0-\u30ffa-zA-Z0-9\s]/g, '')
  ...
  const addMessage = (conversationId: string, message: Message) => {
    ...
    if (conversation) {
      ...
      conversation.lastMessage = message.content.slice(0, 50) + (message.content.length > 50 ? '...' : '')
      conversation.timestamp = new Date().toLocaleString('zh-CN')
      
      // 自动生成标题（仅在第一条用户消息时）
      if (message.sender === 'user' && conversation.messages.filter(m => m.sender === 'user').length === 1) {
  ...
  const createMaskedConversation = (maskName: string, maskDescription: string, maskPrompt: string) => {
    const newConversation: Conversation = {
      id: Date.now().toString(),
      title: maskName,
      lastMessage: '',
